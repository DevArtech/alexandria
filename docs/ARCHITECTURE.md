...
| Milestone | Scope |
| --- | --- |
| **M1 — Skeleton** | Store (Markdown + frontmatter, all tiers incl. `provisional`/`relational`/`threads` dirs), SQLite index with metadata + FTS5, `remember` / `recall` (lexical only), `reindex`. Local `fastembed` wired but optional. |
| **M2 — Hybrid + budget + five-state** | `sqlite-vec` semantic search, RRF fusion, **five-state recall** + density check, progressive-disclosure context tree, `expand`, token-budget API, `--format json`. |
